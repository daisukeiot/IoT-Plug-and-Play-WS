...
static const char *deviceSasKey;
...
static const char *digitalTwinSample_CustomProvisioningData = "{"
                                                              "\"__iot:interfaces\":"
                                                              "{"
                                                              "\"CapabilityModelId\": \"urn:st:stbox1:azure1:1\" ,"
...
// device-first-provisioning Start
    ...
    if ((decode_key = Azure_Base64_Decode(sasKey)) == NULL)
    {
    ...
    {
        if ((HMACSHA256_ComputeHash(BUFFER_u_char(decode_key), BUFFER_length(decode_key), (const unsigned char*)deviceId, strlen(deviceId), hash)) != HMACSHA256_OK)
        {
            (void)printf("Failure decoding group key\r\n");
        }
        else
        {
            ...
            deviceSasKey = STRING_c_str(device_key);
            result = 0;
        }
        BUFFER_delete(hash);
        BUFFER_delete(decode_key);
    ...
    if (strlen(deviceSasKey) == 0)
    {
        (void)printf("Failure generate device Sas Key\r\n");
        return result;
    }
// device-first-provisioning End
...
// device-first-provisioning Start
    if (prov_dev_set_symmetric_key_info(deviceId, deviceSasKey) != 0)
...
int main(int argc, char *argv[])
{
    ...
    {
        LogError("USAGE: IoTinActionChicago [Device ID] [DPS ID Scope] [DPS group symmetric key]");
        return 1;
    }
